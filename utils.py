...
def change_user_role(room_id, account_id, role):
    ...
    headers = {"X-ChatWorkToken": CHATWORK_API_TOKEN}
    data = {
        "members_admin_ids": str(account_id) if role == 'admin' else '',
        "members_member_ids": str(account_id) if role == 'member' else '',
        "members_readonly_ids": str(account_id) if role == 'readonly' else ''
    }
    try:
