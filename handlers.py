...
from constants import SINGLE_EMOJI_PATTERN, JAPANESE_CITIES, CHATWORK_API_TOKEN, MY_ACCOUNT_ID

# --- グローバル変数 ---
omikuji_history = {}

def handle_webhook_event(webhook_event, is_manual=False):
    ...
    if cleaned_body == "/test":
        ...
        handle_ranking_all_command(room_id, account_id, message_id, is_manual)
    ...
    if CHATWORK_API_TOKEN:
        import requests
        try:
            room_info_res = requests.get(f"https://api.chatwork.com/v2/rooms/{room_id}", headers={"X-ChatWorkToken": CHATWORK_API_TOKEN}).json()
            room_info = {"name": room_info_res.get("name", "不明")}
...
def handle_room_info_command(room_id, account_id, message_id, parts):
    ...
    try:
        ...
        logging.error(f"Error handling /roominfo: {e}")
